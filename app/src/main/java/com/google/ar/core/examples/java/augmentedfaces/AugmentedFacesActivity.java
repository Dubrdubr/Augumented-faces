...
public class AugmentedFacesActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  // Рендеринг. Рендереры создаются здесь и инициализируются при создании поверхности GL.
  ...
  // Здесь выделяется временная матрица, чтобы уменьшить количество выделений для каждого кадра.
  ...
  protected void onResume() {
    ...
    if (session == null) {
      ...
      try {
        ...
        // Настройте сеанс для использования фронтальной камеры.
    ...
    // Обратите внимание, что порядок имеет значение - см. Примечание в onPause (), здесь применяется обратное.
    try {
      session.resume();
  ...
  public void onPause() {
    ...
    if (session != null) {
      ...
      // Обратите внимание, что порядок имеет значение - GLSurfaceView сначала приостанавливается,
      // чтобы не пытаться запросить сеанс. Если сеанс приостановлен перед GLSurfaceView,
      // GLSurfaceView все равно может вызвать session.update () и получить исключение SessionPausedException.
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    // Подготовьте объекты рендеринга. Это связано с чтением шейдеров, поэтому может возникнуть исключение IOException.
    try {
      // Create the texture and pass it to ARCore session to be filled during update().
      // Создайте текстуру и передайте ее сеансу ARCore для заполнения во время update ().
      ...
      rightEarObject.createOnGlThread(
          this, "models/forehead_right.obj", "models/tg.png");
  ...
  public void onDrawFrame(GL10 gl) {
    ...
    // Очистите экран, чтобы уведомить водителя, что он не должен загружать пиксели из предыдущего кадра.
    ...
    //Сообщите сеансу ARCore, что размер представления изменился,
    // чтобы можно было правильно настроить матрицу перспективы и фон видео.
    ...
    try {
      ...
      When the configuration is set to UpdateMode.BLOCKING (it is by default),
      this will throttle the rendering to the camera framerate.*/
      // Получить текущий кадр из ARSession.
      // Если для конфигурации установлено значение UpdateMode.BLOCKING (по умолчанию),
      // это ограничит рендеринг частотой кадров камеры.
      Frame frame = session.update();
      Camera camera = frame.getCamera();

      // Get projection matrix.
      // Получите матрицу проекции.
      float[] projectionMatrix = new float[16];
      camera.getProjectionMatrix(projectionMatrix, 0, 0.1f, 100.0f);

      // Get camera matrix and draw.
      // Достаньте матрицу камеры и рисуйте.
      float[] viewMatrix = new float[16];
      camera.getViewMatrix(viewMatrix, 0);

      // Compute lighting from average intensity of the image.
      // Вычислите освещение по средней интенсивности изображения.
      // The first three components are color scaling factors.
      // Первые три компонента - это коэффициенты масштабирования цвета.
      // The last one is the average pixel intensity in gamma space.
      // Последний - это средняя интенсивность пикселей в гамма-пространстве.
      final float[] colorCorrectionRgba = new float[4];
      frame.getLightEstimate().getColorCorrection(colorCorrectionRgba, 0);

      // If frame is ready, render camera preview image to the GL surface.
      // Если рамка готова, визуализируйте изображение предварительного просмотра камеры на поверхность GL.
      backgroundRenderer.draw(frame);

      // Keep the screen unlocked while tracking, but allow it to lock when tracking stops.
      // Держите экран разблокированным во время отслеживания, но позвольте ему заблокироваться, когда отслеживание остановится.
      trackingStateHelper.updateKeepScreenOnFlag(camera.getTrackingState());

      // ARCore's face detection works best on upright faces, relative to gravity.
      // Распознавание лиц ARCore лучше всего работает на вертикальных лицах относительно силы тяжести.
      // If the device cannot determine a screen side aligned with gravity, face
      // detection may not work optimally.
      // Если устройство не может определить сторону экрана,
      // выровненную под действием силы тяжести, функция распознавания лиц может работать неоптимально.
      ...
      for (AugmentedFace face : faces) {
        ...
        // Объекты Face используют прозрачность, поэтому они должны быть отрисованы задом наперед без записи глубины.
        GLES20.glDepthMask(false);

        // Each face's region poses, mesh vertices, and mesh normals are updated every frame.
        // Поза каждой области лица, вершины сетки и нормали сетки обновляются каждый кадр.

        // 1. Render the face mesh first, behind any 3D objects attached to the face regions.
        // 1. Сначала визуализируйте сетку лица за любыми 3D-объектами, прикрепленными к областям лица.
        ...
        // 2. Затем визуализируйте 3D-объекты, прикрепленные ко лбу.
        ...
        // 3. Визуализируйте нос в последнюю очередь,
        // чтобы он не был закрыт сеткой лица или трехмерными объектами, прикрепленными к областям лба.
      ...
      // Избегайте сбоев приложения из-за необработанных исключений.
